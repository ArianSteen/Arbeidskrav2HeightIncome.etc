...
---
```{r include=FALSE}
# Kan være lurt å ha med kommandoen under da dette gir hele tall visuelt og ikke "scientific notation". 
options(scipen=999) 
library(citr)
library(tinytex)
library(knitr)
...
Vi gjør oppmerksom på at dette datasettet ikke inkluderer etnisitet, familiens formue eller hvor de utvalgte bor. 
Datasettet er tatt fra det amerikanske arbeidsmarkedet, det må tas hensyn til dette dersom vi skal bruke dette datasettet som et utgangspunk for andre land og verdensdeler. 
Analysen vil dermed se på hvordan høyde korrelerer med inntekt når det gjelder det amerikanske arbeidsmarkedet. 
...
Pearson korrelasjonskoeffisient måler, samvariasjonen mellom to variabler ved å dele variablenes kovarians på produktet av variablens respektive standardavvik. 
Rho har verdi mellom +1 og -1. 
En verdi på +1 er fullstendig positiv lineær korrelasjon, 0 er ingen lineær korrelasjon, og -1 er fullstendig negativ korrelasjon.
...
Derimot finnes det en korrelasjon mellom utdanning og inntekt, ifølge Pearson testen fikk vi en verdi på *0.393*. 
Ifølge Spearman testen fikk en *rho på 0.406* dette er en klar korrelasjon
...
Vi begynner med et scatterplot. 
Et spredningsplot vil vise verdien av to variabler i et datasett, vi vil visualisere hvor mye den ene variablen (høyde) vil påvirke den andre variablen (inntekt). 
Dette er spearman korrelasjonen visualisert grafisk. 
Man kan se at det ikke er noen sterk sammenheng mellom høyde og inntekt, utenom noen få outliers.
...
Vi kan se at utdanning er positivt korrelert med inntekt, det vil si at du tjener mer dersom du har høyere utdanning. 
Dette kan sammenlignes med høyde hvor korrelasjonen ikke er like positiv. 
Legg også merke til outliers på toppen av grafen. 

# Regresjonslinje og scatterplot

# Histogram og Density plot

Et histogram er en grafisk framstilling, som brukes til å analysere og presentere data. 
Høyden av en søyle er frekvensen delt på klassebredden. 
Vi skal bruke et histogram til å analysere våre ulike variabler. 
Samt trekke en konklusjon basert på funnet. 
...
Konklusjonen er at de aller fleste observasjoner, ligger trukket sammen uten en klar sammenheng. 
Vi ser også at det finnes noen få "outliers" i vårt datasett.
Vi kan ikke trekke en klar konklusjon om at Høyde påvirker inntekt i noen særlig grad.
...
```{r echo=FALSE}
# Dere trenger ikke lage Heightvariable her. Bare tøm 
# heights inn i pipe og så konverter variablene når 
# det trengs (og om det i det hele tatt trengs)
